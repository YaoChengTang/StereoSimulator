...
meta_root="./cache"
model_path="/mount_points/nas/Qwen2-VL-72B-Instruct"
...
tail -n +2 "$CSV_FILE" | split -l $LINES_PER_GPU - ./cache/${prefix}_part_

# Add the header to each part file
for PART_FILE in ./cache/${prefix}_part_*; do
...
process_part() {
    ...
    DEBUG_ENVS=$DEBUG_ENVS CUDA_VISIBLE_DEVICES=$GPU_ID python filter_bad_frames.py --video_path_csv "$PART_FILE" --gpu_id "$GPU_ID" --max_queue_size 10 --frames_root "$frames_root" --videos_root "$videos_root" --max_workers "$max_workers" --num_producers "$num_producers" --meta_root "$meta_root" --step "$step" --model_path "$model_path"
}
...
PART_FILES=$(ls ./cache/${prefix}_part_*)
...
parallel --jobs $NUM_GPUS --line-buffer --no-notice process_part {1} {2} {3} {4} {5} {6} {7} {8} {9} ::: "${PAIRS[@]}" ::: $frames_root ::: $videos_root ::: $max_workers ::: $num_producers ::: $meta_root ::: $step ::: $DEBUG_ENVS ::: $model_path

# Clean up the part files
rm ./cache/${prefix}_part_*