...
def main(
    ...
    for frame_folder in video_paths:
        ...
        # if save_path == "/data5/fooling-depth/depth/video0/putty_texture_paint_how_to_wall_putty_texture_short":
        #     flag = True
        #     # continue
        # else:
        #     continue  
        ...
        #     continue
        # print(save_path)
        # depth_list = os.listdir(save_path)
        # print(len(depth_list), len(frame_names))
        # break
        # continue
        try:
            depth_list = os.listdir(save_path)
        except:
            print(f"Folder {save_path} not found!")
            continue
        print(save_path)
        if not check(os.listdir(save_path)):
            print(f"Folder {save_path} is already renamed!")
            continue
        assert len(depth_list) == len(frame_names)
        # print(save_path)
        # rename_frames(save_path, frame_names)
        # break
